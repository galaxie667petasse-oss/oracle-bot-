...
def build_pr2(lessons):
    import random
    # Varier le type de pari suggere pour forcer la diversite
    pari_examples = random.sample([
        "BTTS Oui (les deux equipes marquent)",
        "Plus de 2.5 buts",
        "Moins de 2.5 buts",
        "Handicap -1 equipe favorite",
        "Mi-temps nul / Victoire finale equipe X",
        "Victoire domicile",
        "Victoire exterieur (si value)",
        "Match nul",
    ...
    s = (
        "Tu es le Professeur Visionnaire, expert absolu en value betting.\n\n"
        "ANALYSE le match en 50 mots max, puis ecris le VERDICT FINAL.\n\n"
        "REGLES ABSOLUES:\n"
        f"- Considere ces types de paris (choisis le meilleur): {', '.join(pari_examples)}, ou autre\n"
        "- Ne choisis PAS forcement la victoire domicile — cherche la VALUE reelle\n"
        "- CONFIANCE doit refleter ta VRAIE certitude : 62=incertain, 70=bon, 80=fort, 88=tres fort\n"
        "- Deux matchs ne peuvent pas avoir la meme CONFIANCE — varie selon l'analyse\n"
        "- COTE_MINI doit correspondre au pari choisi (victoire=1.5-2.5, BTTS=1.6-2.0, over=1.6-2.2)\n"
        "- MISE_PCT : 1=faible confiance, 2=normal, 3=bon, 4=fort, 5=exceptionnel\n\n"
        "VERDICT (ecris EXACTEMENT ces lignes avec de VRAIES valeurs):\n"
        "PARI: [pari precis et specifique]\n"
        "CONFIANCE: [chiffre entre 58 et 87]\n"
        "MISE_PCT: [chiffre entre 1 et 5]\n"
        "COTE_MINI: [cote decimale ex: 1.75]\n"
        "RISQUE: [risque principal en une phrase courte]\n"
        "RESUME: [3 phrases logiques et precises justifiant ce pari]"
    )
    if lessons:
        s += "\n\nLECONS PRECEDENTES (applique-les):\n"
        s += "\n".join(f"- {l['text']}" for l in lessons[-5:])
    return s

def parse_verdict(text: str) -> dict:
    if not text:
        return {"pari":"Analyse indisponible","conf":60,"mp":2,"cm":1.80,"risque":"","resume":""}
    ...
    # Fallback si pari vide
    if not pari:
        for rx in [
            r"(?:recommande|meilleur pari|pari optimal)\s*:?\s*(.{5,60})",
            r"(BTTS|Over \d|Under \d|Victoire|Plus de|Moins de|Handicap)[^\n]{0,50}",
        ]:
            m = re.search(rx, t, re.IGNORECASE)
            if m:
                pari = m.group(0 if "BTTS" in rx else 1).strip()[:80]
                break
    if not pari:
        pari = "Voir analyse"

    # Valeurs numeriques
    try: conf = min(88, max(60, int(cs)))
    except: conf = 65
    ...
    if not resume:
        ...
        resume = " ".join(lines[:3]) if lines else ""

    log.info(f"Verdict OK — pari={pari[:40]}, conf={conf}, cm={cm}")
...
async def analyze_match(match: dict, lessons: list, pcb) -> dict:
    ...
    try:
        reports["pr2"] = await call_groq(
            AGENTS[8]["s"],
            f"{base}\n\nJUGE:\n{reports['juge']}\n\nPRAGMATIQUE:\n{reports['pr1']}\n\n"
            f"Donne ton analyse puis le VERDICT FINAL obligatoire.",
            550
        )
    except Exception as e:

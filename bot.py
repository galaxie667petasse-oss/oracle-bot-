        ...
        for sport_key, comp_name in SPORT_KEYS.items():
            try:
                params = {
                    "apiKey":     ODDSPAPI_KEY,
                    "sportId":    sport_key,
                    "bookmakers": "winamax,bet365,unibet,pinnacle",
                    "markets":    "h2h",
                }
                async with session.get(
                    ...
                    if resp.status == 401:
                        raise RuntimeError("ERREUR: Cle ODDSPAPI invalide")
                    if resp.status != 200:
                        log.warning(f"OddsPapi {resp.status} — {sport_key}")
                        continue
                    data = await resp.json()
                ...
                for event in data.get("data", []):
                    ...
                    for bk in ["winamax","bet365","unibet","pinnacle"]:
                        bk_data = event.get("bookmakerOdds",{}).get(bk,{})
                ...
                log.warning(f"OddsPapi {sport_key}: {e}")

    matches.sort(key=lambda m: m.get("heure","99:99"))
    log.info(f"OddsPapi: {len(matches)} matchs pour {iso_date}")
    return matches[:12]

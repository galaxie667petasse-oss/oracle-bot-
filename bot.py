...
def kelly_criterion(conf: float, cm: float, bankroll: float) -> float:
    """Calcule la mise optimale selon Kelly Criterion.
    conf = probabilite estimee (0-1)
    cm = cote decimale
    retourne la mise en euros."""
    if cm <= 1.0 or conf <= 0 or conf >= 1:
        return 0
    b = cm - 1  # profit net si gagne
    q = 1 - conf
    kelly = (b * conf - q) / b
    kelly = max(0, min(kelly, 0.25))  # Cap a 25% max
    # Demi-Kelly pour prudence
    half_kelly = kelly * 0.5
    mise = round(bankroll * half_kelly, 2)
    return max(1.0, min(mise, bankroll * 0.10))  # Entre 1€ et 10% bankroll

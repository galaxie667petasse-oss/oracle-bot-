...
async def fetch_matches(label):
    ...
    # Essai 1 : Gemini avec Google Search grounding
    # Essai 2 : Gemini sans search (connaissance calendrier)
    prompts_and_tools = [
        (True,  f"Recherche sur Google les matchs football confirmes le {label}. Toutes ligues europeennes."),
        (False, f"Tu es expert football 2025-2026. Liste les matchs programmes le {label} en Europe."),
    ]

    base_json = (
        f'{{"matches":[{{"home":"X","away":"Y","competition":"Ligue 1",'
        f'"date":"{label}","heure":"20:45","cote_home":1.85,'
        f'"cote_draw":3.40,"cote_away":4.20,"contexte":"forme, blesses, enjeux"}}]}}'
    )

    for use_search, base_prompt in prompts_and_tools:
        if matches:
            break
        prompt = (
            base_prompt + "\n\n"
            "Retourne UNIQUEMENT ce JSON (10 matchs max) :\n" + base_json
        )
        for attempt in range(3):
            try:
                ...
                resp = await asyncio.to_thread(
                    lambda: model.generate_content(
                        ...
                        generation_config=genai.GenerationConfig(
                            max_output_tokens=2000, temperature=0.3
                        )
                    )
                )
                raw = resp.text.strip()
                logger.info(f"Gemini raw [{use_search}]: {raw[:300]}")

                # Tentatives de parsing JSON
                for fn in [
                    lambda r=raw: json.loads(r),
                    lambda r=raw: json.loads(re.search(r"```(?:json)?\s*([\s\S]*?)```", r).group(1).strip()),
                    lambda r=raw: json.loads(re.search(r'(\{[\s\S]*?"matches"\s*:\s*\[[\s\S]*?\]\s*\})', r).group(1)),
                ]:
                    try:
                        p = fn()
                        if p and isinstance(p.get("matches"), list) and len(p["matches"]) > 0:
                            matches = p["matches"]
                            logger.info(f"Parsed {len(matches)} matches OK")
                            break
                    except:
                        pass
                ...
                # Si parsing echoue, demander reformatage
                m2 = genai.GenerativeModel("gemini-1.5-flash")
                r2 = await asyncio.to_thread(
                    lambda r=raw: m2.generate_content(
                        f"Convertis ces donnees en JSON valide :\n{base_json}\n\nDonnees brutes:\n{r[:2000]}",
                        generation_config=genai.GenerationConfig(max_output_tokens=1500)
                    )
                )
                rt = r2.text.strip()
                for fn in [
                    lambda r=rt: json.loads(r),
                    lambda r=rt: json.loads(re.search(r'(\{[\s\S]*?"matches"[\s\S]*?\]\s*\})', r).group(1)),
                ]:
                    try:
                        p = fn()
                        if p and isinstance(p.get("matches"), list) and len(p["matches"]) > 0:
                ...
                logger.warning(f"Gemini search={use_search} attempt {attempt+1}: {e}")

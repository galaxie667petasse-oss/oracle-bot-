...
def build_pr2(lessons):
    s = (
        "Tu es le Professeur Visionnaire, expert absolu en paris sportifs.\n"
        "Apres une courte analyse (50 mots max), tu dois imperativement ecrire\n"
        "le bloc VERDICT ci-dessous avec des vraies valeurs (pas des exemples).\n\n"
        "REGLES IMPORTANTES:\n"
        "- Le pari peut etre NIMPORTE QUOI de rentable: victoire, nul, over/under, BTTS,\n"
        "  handicap, mi-temps, premier buteur, nombre de corners, cartons, etc.\n"
        "- Choisis le pari avec le MEILLEUR rapport qualite/value, pas forcement victoire\n"
        "- CONFIANCE entre 60 et 88 selon ta certitude reelle\n"
        "- COTE_MINI realiste selon le pari choisi\n\n"
        "VERDICT OBLIGATOIRE (ecris exactement ces mots):\n"
        "PARI: [ton pari precis]\n"
        "CONFIANCE: [chiffre]\n"
        "MISE_PCT: [1 a 5]\n"
        "COTE_MINI: [cote decimale]\n"
        "RISQUE: [risque principal en une phrase]\n"
        "RESUME: [explication logique en 3 phrases claires et precises]"
    )
    if lessons:
        s += "\n\nLECONS DES PARIS PRECEDENTS:\n"

...
async def fetch_matches(iso_date: str, label: str) -> list:
    """1 seule requete OddsPapi pour tous les matchs football du jour."""
    if not ODDSPAPI_KEY:
    ...
    # Mapping sport_key OddsPapi -> nom competition
    COMP_MAP = {
        "soccer_france_ligue1":           "Ligue 1",
        "soccer_france_ligue2":           "Ligue 2",
        "soccer_england_premier":         "Premier League",
    ...
    # Une seule requete avec sportId=10 (soccer global) et dateFilter
    # OddsPapi supporte le filtrage par date via commenceTimeFrom/commenceTimeTo
    async with aiohttp.ClientSession() as session:
        try:
            params = {
                "apiKey":              ODDSPAPI_KEY,
                "sportId":             10,  # Soccer global
                "bookmakers":          "winamax,bet365,unibet",
                "markets":             "h2h",
            ...
            async with session.get(
                ...
                if resp.status == 429:
                    raise RuntimeError("ERREUR: Limite OddsPapi atteinte (250/mois)")
                if resp.status == 400:
                    # Fallback: essayer sans filtre de date
                    log.warning("OddsPapi 400 avec date filter — tentative sans filtre")
                    params2 = {
                        "apiKey":     ODDSPAPI_KEY,
                        "sportId":    10,
                        "bookmakers": "winamax,bet365,unibet",
                    ...
                    async with session.get(
                        ...
                        if resp2.status != 200:
                            ...
                            raise RuntimeError(f"ERREUR OddsPapi {resp2.status}: {txt[:200]}")
                        data = await resp2.json()
                elif resp.status != 200:
                    txt = await resp.text()
                    raise RuntimeError(f"ERREUR OddsPapi {resp.status}: {txt[:200]}")
                else:
                    data = await resp.json()

        except RuntimeError: raise
        except Exception as e:
            raise RuntimeError(f"ERREUR OddsPapi connexion: {e}")
            ...
            if not comp_name:
                ...
                for k, v in COMP_MAP.items():
                    if v.lower() in league_name.lower():
                        comp_name = v; break
            if not comp_name: continue  # Pas dans nos ligues

            home = event.get("homeTeam", {}).get("name", "?")
            away = event.get("awayTeam", {}).get("name", "?")

            # Extraire les meilleures cotes 1X2
            ch = cd = ca = bk_used = None
            bk_odds = event.get("bookmakerOdds", {})
            for bk in ["winamax", "bet365", "unibet", "pinnacle"]:
    ...
    log.info(f"OddsPapi: {len(matches)} matchs trouves pour {iso_date}")
    return matches[:12]

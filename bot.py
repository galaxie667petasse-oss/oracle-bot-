...
async def fetch_team_stats(session, headers, team_id: int, league_id: int) -> dict:
    ...
    DOMESTIC_FALLBACK = [39,61,140,135,78,94,88,203,40,62]
...
async def enrich_match(session, headers, match: dict) -> dict:
    """Enrichit un match avec stats + blesses + vraies cotes + compos."""
    ...
    base_tasks = [
        fetch_team_stats(session, headers, home_id, league_id),
        fetch_team_stats(session, headers, away_id, league_id),
        fetch_h2h(session, headers, home_id, away_id),
        fetch_last_lineups(session, headers, home_id),
        fetch_last_lineups(session, headers, away_id),
    ]
    if fixture_id:
        base_tasks += [
            fetch_injuries(session, headers, fixture_id),
            fetch_real_odds(session, headers, fixture_id),
        ]
    if not is_euro:
        base_tasks += [
            fetch_standings_position(session, headers, home_id, league_id),
            fetch_standings_position(session, headers, away_id, league_id),
        ]
    ...
    if is_euro:
        home_pos = "Coupe europeenne"
        away_pos = "Coupe europeenne"
    else:
        home_pos = safe(idx,   "?")
        away_pos = safe(idx+1, "?")

    # Remplacer les cotes si vraies cotes disponibles
    if real_odds.get("home"):
...
def build_match_context(match: dict) -> str:
    """Contexte complet avec vraies stats pour les agents."""
    home = match["home"]
    away = match["away"]
    ...
    ctx = (
        ...
        f"=== {home} (DOMICILE) ===\n"
        f"Classement: {hp}\n"
        f"Forme 5 derniers: {hs.get('form','?????')}\n"
        f"Bilan: {hs.get('wins','?')}V {hs.get('draws','?')}N {hs.get('losses','?')}D ({hs.get('played','?')} matchs)\n"
        f"Buts moy: {hs.get('goals_for_avg','?')} marques / {hs.get('goals_against_avg','?')} encaisses\n"
        f"A domicile: {hs.get('home_win_rate','?')} victoires\n"
        f"Derniere compo: {home_lineup}\n\n"

        f"=== {away} (EXTERIEUR) ===\n"
        f"Classement: {ap}\n"
        f"Forme 5 derniers: {as_.get('form','?????')}\n"
        f"Bilan: {as_.get('wins','?')}V {as_.get('draws','?')}N {as_.get('losses','?')}D ({as_.get('played','?')} matchs)\n"
        f"Buts moy: {as_.get('goals_for_avg','?')} marques / {as_.get('goals_against_avg','?')} encaisses\n"
        f"A l'exterieur: {as_.get('away_win_rate','?')} victoires\n"
        f"Derniere compo: {away_lineup}\n\n"

        f"=== BLESSES / SUSPENDUS (TRES IMPORTANT) ===\n"
        f"{injuries}\n\n"
        ...
        f"=== COTES BOOKMAKERS ===\n"
        f"Dom ({home}): {ch} | Nul: {cd} | Ext ({away}): {ca}\n"
        f"Prob. implicite: Dom {prob(ch)}% | Nul {prob(cd)}% | Ext {prob(ca)}%\n"
        f"(Value bet = ta proba reelle > proba implicite)"
    )
    return ctx
...
def build_pr2(lessons):
    ...
    return (
        ...
        "IMPORTANT: Tu DOIS toujours proposer un pari, meme si les stats sont incompletes.\n"
        "Utilise les cotes du marche et le contexte de la competition pour decider.\n"
...
def parse_verdict(text: str) -> dict:
    ...
    try:
        ...
        # Nettoyer le pari: enlever "EquipeA vs EquipeB :" si present
        pari = re.sub(r'^[^:]+\s*:\s*', '', pari).strip() if ':' in pari[:50] else pari
        # Enlever doublons de noms d equipes
        pari = re.sub(r"(?i)(victoire de |victoire d.)", "Victoire ", pari)

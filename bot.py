...
async def fetch_matches(label):
    ...
    prompt = (
        f"Tu es un expert du calendrier football europeen saison 2025-2026.\n"
        f"Date cible : {label}\n\n"
        f"Liste les matchs de football programmes pour le {label}.\n"
        ...
        f"Eredivisie, Super Lig, Primeira Liga.\n\n"
        f"Retourne UNIQUEMENT ce JSON (10 matchs max) :\n"
    ...
    for attempt in range(4):
        try:
            # Gemini sans tools - connaissance calendrier 2025-2026
            ...
            raw = resp.text.strip()
            logger.info(f"Gemini raw (attempt {attempt+1}): {raw[:300]}")
            ...
            # Reformatage si parsing echoue
            logger.info("Parsing failed, trying reformat...")
            base_json = (
                f'{{"matches":[{{"home":"X","away":"Y","competition":"Z",'
                f'"date":"{label}","heure":"HH:MM","cote_home":1.8,'
                f'"cote_draw":3.5,"cote_away":4.0,"contexte":"infos"}}]}}'
            )
            r2 = await asyncio.to_thread(
                lambda r=raw: model.generate_content(
                    f"Convertis en JSON valide uniquement:\n{base_json}\n\nDonnees:\n{r[:2000]}",

...
async def fetch_matches(label, api_date):
    ...
    try:
        ...
        # Enrichir le contexte via Groq
        for m in matches:
            try:
                ctx = await call_groq(
                    "Expert football 2025-2026. Une phrase courte.",
                    f"Contexte rapide pour {m['home']} vs {m['away']} ({m['competition']}) "
                    f"le {label}: forme, blesses, enjeux. Max 20 mots.",
                    60
                )
                m["contexte"] = ctx.strip()
            except Exception:
                pass

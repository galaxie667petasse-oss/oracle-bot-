...
def build_pr2(lessons):
    """Systeme prompt pour le Prof Visionnaire — sortie JSON forcee."""
    lesson_block = ""
    if lessons:
        lesson_block = "\n\nLECONS DES PARIS PRECEDENTS (applique-les absolument):\n"
        lesson_block += "\n".join(f"- {l['text']}" for l in lessons[-5:])
    return (
        "Tu es un expert absolu en value betting football.\n"
        "Tu dois analyser le match et retourner UNIQUEMENT un objet JSON valide, rien d'autre.\n\n"
        "REGLES POUR LE PARI:\n"
        "- Cherche la VRAIE VALUE, pas forcement la victoire domicile\n"
        "- Types possibles: 'Victoire X', 'Victoire Y', 'Match nul', 'BTTS Oui', 'BTTS Non',\n"
        "  'Plus de 2.5 buts', 'Moins de 2.5 buts', 'Plus de 1.5 buts', 'Moins de 1.5 buts',\n"
        "  'Handicap -1 X', 'Double chance X/N', 'Mi-temps nul', 'Plus de 9.5 corners',\n"
        "  'Moins de 3.5 buts', 'X marque en 1ere mi-temps'\n"
        "- Varies OBLIGATOIREMENT les types selon le contexte du match\n\n"
        "REGLES POUR LA CONFIANCE:\n"
        "- 58-64 : match incertain, peu d'infos\n"
        "- 65-69 : leger avantage identifie\n"
        "- 70-74 : bon signal, plusieurs facteurs convergent\n"
        "- 75-79 : fort signal\n"
        "- 80-87 : tres fort, multiples facteurs solides\n"
        "- Ne jamais mettre la meme confiance pour deux matchs differents\n\n"
        "FORMAT DE REPONSE (JSON uniquement, pas de texte autour):\n"
        '{\n'
        '  "pari": "type de pari precis",\n'
        '  "confiance": 72,\n'
        '  "mise_pct": 3,\n'
        '  "cote_mini": 1.85,\n'
        '  "risque": "risque principal en une phrase",\n'
        '  "resume": "3 phrases logiques et precises justifiant ce pari specifique"\n'
        '}' + lesson_block
    )
...
async def analyze_match(match: dict, lessons: list, pcb) -> dict:
    ...
    try:
        reports["pr2"] = await call_groq(
            AGENTS[8]["s"],
            f"{base}\n\nJUGE:\n{reports['juge']}\n\nPRAGMATIQUE:\n{reports['pr1']}\n\n"
            f"Retourne UNIQUEMENT le JSON de ton verdict. Aucun texte avant ou apres.",
            400
        )
    except Exception as e:

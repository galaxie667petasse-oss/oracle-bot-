...
def main():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        direct = shadow_ledger.add_shadow_entry(
            str(ledger),
            ...
            side="away",
            taken_odds="3.10",
            notes="source=api_football; source_event_id=42",
        )
        ...
        with results.open("w", newline="", encoding="utf-8") as fh:
            ...
            writer.writerow({"fixture_id": "42", "date": "2026-06-05", "league": "Serie A", "home_team": "Equipe Divergente", "away_team": "Autre Nom", "home_goals": "0", "away_goals": "1", "is_finished": "True"})
        dry = shadow_result_matcher.match_results(str(ledger), str(results), dry_run=True)
        assert dry["matched"] == 2 and dry["updated"] == 0
        ...
        assert rows[1]["shadow_id"] == direct["shadow_id"]
